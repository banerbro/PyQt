...
class Bird(Game_sprite):
    def __init__(self):
        ...
        self.costumes = [image.load('bird.png'), image.load('bird.png'), image.load('bird.png')]
        self.gravity = 0
    def update(self):
...
class PipeDown(Game_sprite):
    def __init__(self):
        super().__init__(image.load('pipedown.png'), 52, 320, 350, 200)
...
background = transform.scale(image.load('back.png'), (400, 500))
bird = Bird()
pipedown = PipeDown()
pipeup = PipeUp()
...
font.init()
f = font.Font(None, 36)
scoretext = f.render(str(score), 1, (255, 100, 10))
...
while game:
    ...
    if not(finish):
        ...
        score += 0.1
        scoretext = f.render(str(int(score)), 1, (255, 100, 10))
